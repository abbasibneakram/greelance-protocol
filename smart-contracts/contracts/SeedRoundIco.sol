...
contract ICO is ReentrancyGuard {
    ...
    function buyWithEth() public payable noReentrant {
        ...
        uint256 immediateTokens = (tokensToBuy * immediatePercentageReleased) /
            100;
    ...
    function buyWithToken(
        ...
        uint256 tokensToBuy;
        token == dai
            ? tokensToBuy = grlOfDai(_amount)
            : tokensToBuy = grlOfUsdt(_amount);
        ...
        require(
            ...
            "You must have enough Tokens to buy GRL!"
        );
        require(
            token.transferFrom(msg.sender, fundReceiver, _amount),
            "You must Deposit some tokens"
        );
        uint256 immediateTokens = (tokensToBuy * immediatePercentageReleased) /
            100;
        uint256 tokenToVest = tokensToBuy - immediateTokens;
    ...
    function getGrlPrice()
        public
        view
        returns (uint256 grlDaiPrice, uint256 grlEthPrice, uint256 grlUsdtPrice)
    {
        ...
        if (
            currentTime < startTime + phaseDuration * (currentPhase + 1) ||
            tokensSold <= tokensPerPhase * (currentPhase + 1)
        ) {
